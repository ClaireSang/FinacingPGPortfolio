    ...
    def _build_network(self, layers):
        ...
        for layer_number, layer in enumerate(layers):
            ...
            #     self.add_layer_to_dict('softmax_layer', network, weights=False)
                ...
                # self.add_layer_to_dict(layer["type"], network, weights=False)
                btc_bias = tf.tile(btc_bias, [self.input_num, 1])
                self.add_layer_to_dict('tanh_layer',btc_bias,weights=False)
                btc_bias = tflearn.layers.core.activation(btc_bias,activation="tanh")
                self.add_layer_to_dict("pre_softmax_layer", network,weights=False)
                network = tflearn.layers.core.activation(network,activation="softmax")
                #重新計算比例
                finacing_ratio = (tf.multiply(network, (1 - btc_bias)))
                #接在一起
                network = tf.concat([btc_bias, finacing_ratio], 1)
                # self.voting = final_network
                # self.add_layer_to_dict('voting', final_network, weights=False)
                # network = tflearn.layers.core.activation(final_network, activation="softmax")
                # self.add_layer_to_dict('softmax_layer', final_network, weights=False)
