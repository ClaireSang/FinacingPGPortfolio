...
class NNAgent:
    def __init__(self, config, restore_dir=None, device="cpu"):
        ...
        #test
        self.__interests_rate = self.__config["trading"]["interests_rate"]
        #
    ...
    #test
    @property
    def interests_rate(self):
        return self.__interests_rate
    ...
    def __set_loss_function(self):
        ...
        def loss_function8():
            # return -tf.reduce_mean(tf.log(self.pv_vector - tf.clip_by_value(self.__net.output[0, 0], -1, 0)*self.__interests_rate ))
            return -tf.reduce_mean(
                tf.log(tf.reduce_sum(self.__net.output[:] * self.__future_price, reduction_indices=[1]) \
                       - tf.reduce_sum(tf.abs(self.__net.output[:, 1:] - self.__net.previous_w)* self.__commission_ratio, reduction_indices=[1]) \
                       -tf.reduce_sum(tf.abs(tf.clip_by_value(self.__net.output[0, 0], -1, 0)*self.__interests_rate ))))
    ...
    def __pure_pc(self):
        ...
        mu = 1 - (tf.reduce_asum(tf.abs(w_t1[:, 1:]-w_t[:, 1:]), axis=1)*c+tf.reduce_sum(tf.clip_by_value(w_t1[:,1:],-1,0)-tf.clip_by_value(w_t[:,1:],-1,0))*i)
        """
        mu = 1-3*c+c**2
